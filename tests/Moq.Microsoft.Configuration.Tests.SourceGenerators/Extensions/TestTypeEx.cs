		...
		public static TypeDetails[] CreateDetails(this Array @this)
		{
			...
			for (var i = 0; i < @this.Length; i++)
			{
				...
				array[i] = val switch
				{
					...
					TestType.IntUnsignedNullable => new TypeDetails(val, true) { DeclarationName = "uint?", ValueTexts = UintValueTexts },
					...
					TestType.ByteSignedNullable => new TypeDetails(val, true) { DeclarationName = "sbyte?", ValueTexts = SbyteValueTexts },
					TestType.Short => new TypeDetails(val) { DeclarationName = "short", ValueTexts = ShortValueTexts },
					TestType.ShortNullable => new TypeDetails(val, true) { DeclarationName = "short?", ValueTexts = SbyteValueTexts },
					TestType.ShortUnsigned => new TypeDetails(val) { DeclarationName = "ushort", ValueTexts = UshortValueTexts },
					TestType.ShortUnsignedNullable => new TypeDetails(val, true) { DeclarationName = "ushort?", ValueTexts = UshortValueTexts },
					TestType.Long => new TypeDetails(val) { DeclarationName = "long", ValueTexts = LongValueTexts },
					TestType.LongNullable => new TypeDetails(val, true) { DeclarationName = "long?", ValueTexts = LongValueTexts },
					TestType.LongUnsigned => new TypeDetails(val) { DeclarationName = "ulong", ValueTexts = UlongValueTexts },
					TestType.LongUnsignedNullable => new TypeDetails(val, true) { DeclarationName = "ulong?", ValueTexts = UlongValueTexts },
					TestType.Decimal => new TypeDetails(val) { DeclarationName = "decimal", ValueTexts = DecimalValueTexts },
					TestType.DecimalNullable => new TypeDetails(val, true) { DeclarationName = "decimal?", ValueTexts = DecimalValueTexts },
					TestType.Double => new TypeDetails(val) { DeclarationName = "double", ValueTexts = DoubleValueTexts },
					TestType.DoubleNullable => new TypeDetails(val, true) { DeclarationName = "double?", ValueTexts = DoubleValueTexts },
					TestType.Float => new TypeDetails(val) { DeclarationName = "float", ValueTexts = FloatValueTexts },
					TestType.FloatNullable => new TypeDetails(val, true) { DeclarationName = "float?", ValueTexts = FloatValueTexts },
					TestType.String => new TypeDetails(val, true) { DeclarationName = "string", ValueTexts = StringValueTexts },
